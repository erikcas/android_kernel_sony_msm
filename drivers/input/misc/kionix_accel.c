...
struct kionix_accel_driver {
	...
	int (*kionix_accel_selftest)(struct kionix_accel_driver *acceld);
...
static DEVICE_ATTR(selftest	, S_IRUGO, kionix_accel_selftest,NULL);
				 ...
				 const struct i2c_device_id *id)
{
	...
	switch (err) {
		...
		case KIONIX_ACCEL_WHO_AM_I_KXCJK_1013:
			...
			acceld->kionix_accel_selftest				= kionix_accel_grp4_selftest;
			break;
		default:
			KMSGERR(&acceld->client->dev, \
...
static struct i2c_driver kionix_accel_driver = {
	.driver = {
		...
		.owner	= THIS_MODULE,
		.of_match_table = kionix_accel_dts_table, 
	},
